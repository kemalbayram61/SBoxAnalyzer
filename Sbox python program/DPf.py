...
count = np.zeros((2**m,2**n), dtype=int)
# generate the differential distribution table with the size of 2**m*2**n
...
# for a delta x, get the maximum ones of all delta y and generate the values of differential distribution
maxi = np.zeros((2**m,), dtype=int)
for a in range(1,2**m):
    for b in range(2**n):
        if maxi[a-1]<count[a][b]:
            maxi[a-1] = count[a][b]

# put the values in a table of 16*16, so that the table of input/output XOR distribution is obtained
dpbox = np.zeros((16,16), dtype=int)
for i in range(16):
    for j in range(16):
